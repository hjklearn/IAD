...
# def comp_class_pro_with_wei(
#     label: torch.Tensor, 
#     feature: torch.Tensor, 
#     num_classes: int,
#     eps: float = 1e-6,
#     power: float = 0.5,   # 1/count^p, p=0.5更平滑
#     # clip_min: float = 0.2,
#     # clip_max: float = 5.0,
...
#     Returns:
#         prototypes: (B, num_classes, C) tensor
#         class_counts: (B, num_classes) tensor
#         weights: (B, num_classes) tensor
#     """
#     B, C, H, W = feature.shape
#     prototypes = torch.zeros((B, num_classes, C), device=feature.device)
#     class_counts = torch.zeros((B, num_classes), device=feature.device)
#     weights = torch.zeros((B, num_classes), device=feature.device)

#     for b in range(B):
#         for cls in range(num_classes):
#             mask = (label[b] == cls)
#             count = mask.sum().float()
#             class_counts[b, cls] = count
...
#             prototypes[b, cls] = cls_feats.mean(dim=1)
...
#             present_mask[ignore_index] = False
#             counts[ignore_index] = 0.0

#         inv = torch.zeros_like(counts)
#         inv[present_mask] = 1.0 / (counts[present_mask].pow(power) + eps)

#         if present_mask.sum() > 0:
#             inv = inv / (inv[present_mask].mean() + eps)  # 归一化到 mean=1
#             # inv = torch.clamp(inv, min=clip_min, max=clip_max)

#         inv[~present_mask] = 0.0  # 缺失类权重设0
#         weights[b] = inv
...
#     def __init__(self, ksize=5, sigma=1.0, channels=None):
#         super().__init__()
#         grid = torch.arange(ksize) - (ksize-1)/2
#         g1 = torch.exp(-(grid**2)/(2*sigma**2))           # 一维高斯核
#         kernel = (g1[:,None] @ g1[None,:])                 # 二维高斯核 (外积)
#         kernel = kernel / kernel.sum()                     # 归一化
#         self.register_buffer('k', kernel[None,None,:,:])   # 存成(1,1,K,K)的buffer
#         self.ksize = ksize
#         self.channels = channels

#     def forward(self, x):
#         B,C,H,W = x.shape
#         k = self.k.expand(C,1,self.ksize,self.ksize) if self.channels is None \
#             else self.k.expand(self.channels,1,self.ksize,self.ksize)
#         x = F.conv2d(x, k, padding=self.ksize//2, groups=C) # 对每个通道做高斯平滑
#         return x
