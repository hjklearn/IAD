...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1, class_names=None):
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            if x != patch_size[0] or y != patch_size[1]:
                GT_label = zoom(GT_label, (patch_size[0] / x, patch_size[1] / y), order=3)  # previous using 0
            ...
            with torch.no_grad():
                ...
                probs = P.squeeze(0).cpu().numpy()  # [C, H, W]
                # ====== 从真实label中采样每类像素点 ======
                for c in range(1, 4):  # 0通常是背景，可根据需要包含
                    indices = np.argwhere(GT_label == c)
                    if len(indices) < 200:
                        continue
                    sampled_idx = indices[np.random.choice(len(indices), 100, replace=False)]

                    # 获取这些点的特征向量
                    # feat_per_class = np.array([probs[:, y, x] for y, x in sampled_idx])
                    feat_per_class = np.array([probs[:, y, x] for y, x in sampled_idx]).mean(0, keepdims=True)

                    all_features.append(feat_per_class)
                    all_labels.append(np.ones(len(feat_per_class)) * c)                        
    ...
    return metric_list, metric_list_dice, all_features, all_labels
