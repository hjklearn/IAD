...
class Sam(nn.Module):
    ...
    def __init__(
        ...
        # torch.load("/media/lai/data_h/hjk/医学图像分割/Other_dataset/KD_model/segment_anything_Stu/modeling/450_act3_mobilenetv3_large.pth"), 
        # strict=False)
        # print('pretain mobilenetv3 loading!!!')
    ...
    def forward(self, batched_input, multimask_output=0, gt=None, mode='train'):
    ...
    def forward_train(self, batched_input, multimask_output, input_points=None, gt=None, mode='train'):

        input_images = self.preprocess(batched_input)
        fea_x, local_fea = self.image_encoder(input_images)
        # print('fea_x', fea_x.shape)
        image_embedding = self.conv_embed(fea_x)
        # mask_prompt = self.prompt_generator(inter_features, image_embeddings)
        if self.image_size == 224:
            image_embedding = F.interpolate(image_embedding, size=(14, 14), mode='bilinear', align_corners=False)
        ...
        return self.up4(low_res_masks), local_fea, image_embedding
