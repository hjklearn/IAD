import sys
sys.path.append('/media/lai/data_h/hjk/医学图像分割/Other_dataset/KD_model/')
import os
import numpy as np
import torch
...
def test_single_volume(image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            net.eval()
            with torch.no_grad():
                # inputs = F.interpolate(inputs, size=(512, 512), mode='bilinear', align_corners=False)
                outputs1, _, _ = net(inputs, multimask_output)

                # #### 热力图可视化 ######
                # sample = outputs1['low_res_masks_1'][0].sum(dim=0)  # shape: (64, 64)
                # sample = torch.sigmoid(sample)
                # # sample = (sample > 0.5).float()
                # sample = sample.detach().cpu().numpy()
                #
                # plt.imshow(sample, cmap='inferno')  # 可以换成 'viridis', 'jet' 等
                # plt.axis('off')                         # 去掉坐标轴
                # plt.tight_layout(pad=0)                # 紧凑布局
                #
                # save_path = os.path.join('/root/autodl-tmp/H-SAM_new/Tool_Synapse/output_224/camp', f'{case}_{ind}.png')
                ...
                if stage == 3:
                    ...
                    # low_res_masks_1 = outputs1['low_res_masks_1']
                    # output_masks = outputs1['predict3_2']

                    # ## 可视化 ##
                    # summed = torch.sum(outputs1['binary_predict1'], dim=1, keepdim=True)  # 输出形状 (B, 1, H, W)
                    # print('summed', summed.shape)
                    # img = summed[0, 0, :, :]  # 形状 (H, W)
                    # img = img.cpu().numpy()
                    # # 动态归一化到0-255
                    # img_min = np.min(img)
                    # img_max = np.max(img)
                    # img_normalized = (img - img_min) / (img_max - img_min + 1e-8) * 255
                    # img_uint8 = img_normalized.astype(np.uint8)
                    # Image.fromarray(img_uint8).save(f"batch_1_summed.png")
                    # while True:
                    #     pass
                ...
                # ###### mask1 ######
                # low_res_masks_1 = torch.argmax(torch.softmax(low_res_masks_1, dim=1), dim=1).squeeze(0)
                # low_res_masks_1 = low_res_masks_1.cpu().detach().numpy()
                # low_res_masks_1_h, low_res_masks_1_w = low_res_masks_1.shape
                # if x != low_res_masks_1_h or y != low_res_masks_1_w:
                #     low_res_masks_1 = zoom(low_res_masks_1, (x / low_res_masks_1_h, y / low_res_masks_1_w), order=0)
                # else:
                #     low_res_masks_1 = low_res_masks_1
                # mas1_d[ind] = low_res_masks_1

                # ###### mask2 ######
                # low_res_masks_2 = torch.argmax(torch.softmax(low_res_masks_2, dim=1), dim=1).squeeze(0)
                # low_res_masks_2 = low_res_masks_2.cpu().detach().numpy()
                # low_res_masks_2_h, low_res_masks_2_w = low_res_masks_2.shape
                # if x != low_res_masks_2_h or y != low_res_masks_2_w:
                #     low_res_masks_2 = zoom(low_res_masks_2, (x / low_res_masks_2_h, y / low_res_masks_2_w), order=0)
                # else:
                #     low_res_masks_2 = low_res_masks_2
                # mas2_d[ind] = low_res_masks_2
